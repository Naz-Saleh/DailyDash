...
from tables import Notification, db, User, Article, NewsCategory, Bookmark
...
class UserNotificationObserver(Observer):
    ...
    def update(self, count):
        if count > 0:
            msg = f"{count} new articles just arrived!"
            notif = Notification(user_id=self.user_id, message=msg)
            DBFacade.add(notif)
...
class Subject:
    ...
    def notify_observers(self, new_count):
        for observer in self._observers:
            observer.update(new_count)



class FetchStrategy(ABC, Subject):
    """Strategy Interface """
    def __init__(self):
    ...
    def _save_and_count_new(self, raw_articles, category):
        new_count = 0
        updated_count = 0
        ...
        try:
            ...
            # NOTIFY OBSERVERS
            if new_count > 0:
                self.notify_observers(new_count)
...
class RSSFetcher(FetchStrategy):
    ...
    def fetch(self, category, source):
        ...
        BBC_URLS = {'general': "https://feeds.bbci.co.uk/bengali/rss.xml"}
        ...
        self._save_and_count_new(fresh_articles, category)
...
def get_news_headlines(category, region='international', source='all', search_date=None):
    ...
    if should_fetch:
        ...
        for user_row in all_users:
            strategy.attach(UserNotificationObserver(user_row.id))
        

        context = NewsContext(strategy)
        context.execute_fetch(category, source)
    ...
    if form.validate_on_submit():
        ...
        if user and bcrypt.check_password_hash(user.password, form.password.data):
            ...
            if notifications:
                ...
                flash(f"Welcome back! You have unread updates.", "info")
